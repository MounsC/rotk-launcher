/** Package the reviewed ROTK staff voice badge candidate. Offline; no installation writes. */
...
// Published assets-v1.8.1. Preserve the trunk and match-end controls hotfix.
export const SOURCE_CANDIDATE = Object.freeze({
  "assets_x64_0.pack2": { size: 2480212677, sha256: "29cd7f255a3787570b8e8375052e986d231cfdcb4720f9d5ea33e8a121f7684c" },
...
export const CANDIDATE = Object.freeze({
  "assets_x64_0.pack2": { size: 2480695087, sha256: "f1e97eea0e1edd1d60f453830703f12c7336a03439e18d9d7dd0998c8f89d33a" },
...
export function updateManifests(feed, payloads, version, archives, packs) {
  ...
  const main = assets.get("assets_x64_0"), ui = assets.get("rank_menu_ui");
  for (const asset of [main, ui]) assert(asset?.type === "zip" && pathKey(asset.installPath) === pathKey(installPath),
    "published main and rank UI archive required");
  const sourceRows = payloads.files.filter(row => Object.hasOwn(groups, nameKey(row.asset)));
  assert.equal(sourceRows.length, 3, "unexpected source archive ownership");
  for (const [owner, names] of Object.entries(groups)) for (const name of names) {
    const row = sourceRows.find(row => pathKey(row.path) === pathKey(`${installPath}/${name}`));
    assert(row && nameKey(row.asset) === owner, "source pack has wrong owner");
    assert.equal(row.sha256, SOURCE_CANDIDATE[name].sha256, "source pack changed; reconcile that release first");
    assert.equal(row.size, SOURCE_CANDIDATE[name].size, "unexpected source pack size");
  }
  ...
  const changed = Object.entries(groups).map(([name]) => {
    ...
    return { ...(name === "assets_x64_0" ? main : ui), name, version, type: "zip", installPath,
  ...
  const files = payloads.files.filter(row => !Object.hasOwn(groups, nameKey(row.asset)));
  ...
  return {
    feed: { ...feed, packVersion: version, assets: feed.assets.map(asset => changed.find(next => next.name === nameKey(asset.name)) ?? asset) },
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (![directory, feedPath, payloadPath, output, version].every(Boolean) || extra.length) {
    console.error("Usage: node scripts/prepare-staff-voice-assets.mjs <reviewed-packs-directory> <current-feed.json> <current-payloads.json> <new-output-directory> <X.Y.Z>");
    process.exitCode = 2;
  } else {
